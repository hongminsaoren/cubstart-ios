...
@Observable class CardManager {
    ...
    func createCard() -> CFCard {
        ...
        var number = createCreditCardNumber()
        var idx=Int.random(in: 0..<personNames.count)
        var owner = personNames[idx]
        var balance = Int.random(in: -10000...10000)
        var transaction = generateTransactions(withMax: 10, for: number)
        return CFCard(cardNumber: number, ownerName: owner, balance: balance, transactions: transaction)
    }
    ...
    func getCardsPositiveAndNegativeBalances() -> CFCardBalances {
        ...
        var res=CFCardBalances(cardsWithPositiveBalances: [], cardsWithNegativeBalances: [])
        for card in cards{
            if card.balance>0{
                res.cardsWithPositiveBalances.append(card)
            }else{
                res.cardsWithNegativeBalances.append(card)
            }
        }
